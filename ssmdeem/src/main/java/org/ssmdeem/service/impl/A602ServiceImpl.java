...
@Service
public class A602ServiceImpl implements A602Service {
    @Autowired
    private A602Mapper a602Mapper;
    public int insert (A602 record){
        return a602Mapper.insert(record);
    }
//    public List<A602> find(@Param("cityVillage")String cityVillage, @Param("internetbundle")String internetbundle, @Param("userType")String userType){
//        return a602Mapper.find(cityVillage,internetbundle,userType);}
    public  List<A602>find(A602 a602){
        return a602Mapper.find(a602);
    }
    public List<A602> findAll(){
        return  a602Mapper.findAll();
    }
    public int updateByPrimaryKeySelective(A602 record){ return  a602Mapper.updateByPrimaryKeySelective(record);}
    public void importExcelInfo(String filePath) throws IOException, BiffException {

        List<A602> list = new ArrayList<A602>();
...
//                        A602 aa=new A602(one.getStringCellValue());
                        ...
                        user.setQq( four.getStringCellValue());
                        user.setUserType( five.getStringCellValue());
                        user.setBillNumber( six.getStringCellValue());
                        user.setUserType(seven.getStringCellValue());
                        user.setDuration(eight.getStringCellValue());
                        user.setTimes(n9.getStringCellValue());
                        user.setCallcost(n10.getStringCellValue());
                        list.add(user);
                    }
                }
        ...
        a602Mapper.insertInfoBatch(list);
    }
}
